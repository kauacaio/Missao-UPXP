...
with sync_playwright() as pw:
    ...
    def retry_wrong():
        p=game();prime(p)
        p.evaluate("mock.responses['rpc:submit_answer']=[{is_correct:false,points_earned:0}]")
        p.evaluate('submitAnswer(0)')
        check(not p.evaluate('state.challenge.answered'))
        p.locator('[data-action=retry-answer]').click()
        check(p.locator('#confirmAnswer').is_disabled())
        p.evaluate("delete mock.responses['rpc:submit_answer']")
        p.locator('.answer[data-index="1"]').click()
        p.locator('#confirmAnswer').click()
        p.wait_for_timeout(100)
        check(p.evaluate('state.challenge.answered'))
        check(p.evaluate('mock.player.score')==1000)
        p.evaluate('submitAnswer(1)')
        check(p.evaluate("mock.calls.filter(c=>c.key==='rpc:submit_answer').length")==2)
        check(not p.errors,str(p.errors))
    run('wrong answer retries then correct answer stays locked',retry_wrong)
    def duplicate():
